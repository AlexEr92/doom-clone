...
static void draw_char(Framebuffer *fb, int x0, int y0, char ch, uint32_t color, int scale)
{
    ...
    if (ch >= 'A' && ch <= 'Z') {
                ...
                if (dx == 1) {
                    for (int dy = 0; dy < scale; dy++) {
                        for (int dx2 = 0; dx2 < scale; dx2++) {
                            fb->pixels[(size_t)(y0 + r * scale + dy) * SCREEN_W +
                                       (x0 + dx * scale + dx2)] = color;
                        }
                    }
                }
            }
        ...
        for (int dx = 0; dx < 3; dx++) {
            for (int dy = 0; dy < scale; dy++) {
                for (int dx2 = 0; dx2 < scale; dx2++) {
                    fb->pixels[(size_t)(y0 + 2 * scale + dy) * SCREEN_W + (x0 + dx * scale + dx2)] =
                            color;
                }
            }
        }
        ...
        for (int dx = 0; dx < 3; dx++) {
            if (dx == 1) {
                for (int dy = 0; dy < scale; dy++) {
                    for (int dx2 = 0; dx2 < scale; dx2++) {
                        fb->pixels[(size_t)(y0 + scale + dy) * SCREEN_W + (x0 + dx * scale + dx2)] =
                                color;
                        fb->pixels[(size_t)(y0 + 3 * scale + dy) * SCREEN_W +
                                   (x0 + dx * scale + dx2)] = color;
                    }
                }
            }
