...
flowchart TB
    ...
    zb -. "хитскан читает Z-буфер" .-> wp
    sp5 -. "позиции предметов" .-> it
```

Сплошные стрелки — обычный поток кадра. Пунктирные — два места, где
симуляция зависит от рендера; про них ниже, в «Известных ограничениях».
...
синтезируются в `audio.c` как WAV в памяти. Внешний ассет ровно один —
файл карты. `assets_load_png()` написан и не используется: он оставлен,
чтобы процедурную графику можно было заменить файлами.
...
Преследование идёт по прямой, видимость проверяется рейкастом. Поиска пути
нет намеренно: на одной карте 24×24 с редкими препятствиями A* даёт
неотличимый результат при заметно большей сложности.

Стрельба — хитскан: луч из камеры по направлению взгляда, поиск ближайшей
цели, проверка Z-буфера, чтобы не стрелять сквозь стены.
...
Три места, где рендер и симуляция срослись сильнее, чем следует:

- игрок хранит вектор плоскости камеры вместе с игровым состоянием;
- хитскан считает попадания в экранных координатах, читая глобальный
  Z-буфер, — то есть выстрел невозможно посчитать, не нарисовав кадр;
- `SpriteList` служит источником истины для позиций предметов, хотя должен
  быть чисто рендерным списком.

В одиночной игре это работает и никому не мешает. Серверную симуляцию без
кадра делает невозможной — разбор и план развязки в [roadmap.md](roadmap.md).
